...
const Dashboard = ({subscriptionPlan}: PageProps) => {
  ...
  return (
    <main className='max-w-7xl mx-auto md:p-10 '>
      <div className=' justify-between mt-8 items-start flex flex-col gap-4 border-b border-gray-200 pb-5 sm:flex-row sm:items-center sm:gap-0 '>
      ...
      {files && files.length !== 0 ? (
            ...
            <li key={file.id} className=' col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow transition hover:shadow-lg '>
              <Link href={`/dashboard/${file.id}`} className='flex flex-col gap-2'>
                <div className='pt-6 px-6 flex w-full items-center justify-between space-x-6  '>
                  <div className='h-10 w-10 flex-shrink rounded-full bg-gradient-to-r from-violet-400 to-purple-600  ' />
              ...
              <div className='px-6 mt-4 grid grid-cols-3 place-items-center py-2 gap-6 text-xs text-zinc-500 '>
                <div className='flex items-center gap-2'>
                  ...
                  {/* Remember we used date-fns to fix the date */}
                ...
                {/* delete icon, to delete file from database */}
        ...
        <div className='mt-16 flex flex-col items-center gap-2'>
          ...
          <h3 className='font-semibold text-xl'> Hey it seem pretty empty in here.. </h3>
